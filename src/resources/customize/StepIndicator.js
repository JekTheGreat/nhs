	...
	renderProgressBarBackground = () => {
  	...
  	if (direction === "vertical") {
  		...
  		progressBarBackgroundStyle = {
  			backgroundColor: this.state.customStyles.separatorUnFinishedColor,
  			position: "absolute",
  			bottom: (this.state.height - this.state.customStyles.separatorStrokeWidth) / 2,
	...
	renderStepLabels = () => {
		...
		const labelViews = labels.map((label, index) => {
			const selectedStepLabelStyle = index === currentPosition ?
				{ color: this.state.customStyles.currentStepLabelColor } : { color: this.state.customStyles.labelColor };
			...
			return (
				<TouchableWithoutFeedback
					...
					<View style={styles.stepLabelItem}>
						<Text style={[styles.stepLabel, selectedStepLabelStyle,
