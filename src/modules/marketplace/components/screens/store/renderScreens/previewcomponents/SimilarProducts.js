...
export default class SimilarProducts extends React.PureComponent {
    ...
    _renderSimilar = ({ item, index }) => {
        ...
        return (
            <View key={`sim ${index}`} style={{ width: "50%", marginTop: 10 }}>
    ...
    render() {
        const { similarProducts, navigation } = this.props;
        return (
            <View style={{ marginHorizontal: 15, marginVertical: 15 }}>
                {!_.isEmpty(similarProducts) ?
                    <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                        <Text style={{ fontSize: 16, fontWeight: "bold", fontFamily: "Roboto", color: "black" }}>Similar Products</Text>
                        <TouchableOpacity onPress={() => navigation.navigate("SimilarProducts")} style={{ flexDirection: "row", justifyContent: "center", alignItems: "center" }}>
                    ...
                    : <Text style={{ fontSize: 16, fontWeight: "bold", alignSelf: "center", fontFamily: "Roboto-Light", color: Color.Standard2 }}>No Similar Products Showing.</Text>}

                <FlatList
                    data={_.slice(similarProducts, 0, 2)}
                    numColumns={2}
                    showsVerticalScrollIndicator={false}
                    keyExtractor={(item) => `sim ${item.id}`}
                    renderItem={this._renderSimilar} />
            </View>
        )
    }
}
