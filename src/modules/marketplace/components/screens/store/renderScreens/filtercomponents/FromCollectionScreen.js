...
export default class FromCollectionScreen extends React.PureComponent {
    ...
    _renderProducts = ({ item, index }) => {
        const { marketplace: { isFromCollectionScreen } } = this.props;
                ...
                <TouchableOpacity onPress={() => this._goToPreview(item)} style={{
                    ...
                    <View style={{ paddingHorizontal: 10, marginTop: 5, flexDirection: "row", justifyContent: "space-between" }}>
                        ...
                        <Text style={{ alignSelf: "center", textAlign: "center", fontSize: 11, fontFamily: "Roboto-Light", color: Color.Standard2 }}>
                            {`${isFromCollectionScreen ? item.reviews : item.reviews.length} Reviews`}</Text>
                    </View>
                </TouchableOpacity>
            </View>
        )
    ...
    latest = (item) => {
        const data = _.orderBy(item, ["UpdatedAt"], "desc");
    ...
    render() {
        const { marketplace: { setInputDetails, postProductList, setCollectionProducts, isFromCollectionScreen } } = this.props;
        const { active, data } = this.state;
        const arrData = _.isEmpty(data) && isFromCollectionScreen ? setCollectionProducts.product : _.isEmpty(data) && !isFromCollectionScreen ? postProductList.data : data;
        console.log("STATE:", this.state.asc);
        return (
            <View style={{ padding: 15 }}>
                <Text style={{ fontFamily: "Roboto-Light", fontSize: 17, fontWeight: "bold", color: "black" }}>
                    {`${isFromCollectionScreen ? `${setCollectionProducts.name} Collection` : `Search results using Filter.`}`}
                </Text>

                <View style={{ flex: 1, flexDirection: "row", justifyContent: "space-evenly", alignItems: "center", marginLeft: 5, marginTop: 20, backgroundColor: Color.colorPrimaryMP, height: 50, width: "97%" }} >
                    <TouchableOpacity
                        onPress={() => this.relevance(setCollectionProducts.product)}
                    ...
                    <TouchableOpacity
                        onPress={() => this.latest(setCollectionProducts.product)}
                    ...
                    <TouchableOpacity
                        onPress={() => this.topsales(setCollectionProducts.product)}
                    ...
                    <TouchableOpacity
                        onPress={() => this.price(setCollectionProducts.product)}
