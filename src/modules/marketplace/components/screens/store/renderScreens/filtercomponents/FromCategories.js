...
export default class FromCategories extends React.PureComponent {
    ...
    render() {
        const { marketplace: { setInputDetails, postProductList } } = this.props;
        const { active, data } = this.state;
        const arrData = _.isEmpty(data) ? postProductList.data : data;
        console.log("STATE:", this.state.asc);
        return (
            <View style={{ padding: 15 }}>
                <Text style={{ fontFamily: "Roboto-Light", fontSize: 17, fontWeight: "bold", color: "black" }}>
                    Search results using " <Text style={{ fontFamily: "Roboto-Light", fontSize: 17, fontWeight: "bold", color: Color.colorPrimaryMP }}>
                        {setInputDetails.categoryName}</Text> "
                </Text>
                ...
                <View style={{ flex: 1, flexDirection: "row", justifyContent: "space-evenly", alignItems: "center", marginLeft: 5, marginTop: 20, backgroundColor: Color.colorPrimaryMP, height: 50, width: "97%" }} >
                    <TouchableOpacity
                        onPress={() => this.relevance(postProductList.data)}
                    ...
                    <TouchableOpacity
                        onPress={() => this.latest(postProductList.data)}
                    ...
                    <TouchableOpacity
                        onPress={() => this.topsales(postProductList.data)}
                    ...
                    <TouchableOpacity
                        onPress={() => this.price(postProductList.data)}
                ...
                <FlatList
                    ...
                    keyExtractor={(item, index) => `prod_id ${index}`}
                    renderItem={this._renderProducts} />


                {_.isEmpty(arrData) && <View style={{ marginTop: 50, justifyContent: "center", alignItems: "center", }}>
