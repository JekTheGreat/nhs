...
import { PanResponder, Platform, ScrollView,
...
export default class ViewPager extends PureComponent {
    ...
    render () {
    	return (this.props.forceScrollView || Platform.OS === "ios") ? this._renderOnIOS() : (
    		<ViewPagerAndroid
    			...
    			ref={(ref) => this.VIEWPAGER_REF = ref}
    ...
    _renderOnIOS () {
    	...
    	const pagescroll = this.props.onPageScroll ? 8 : 1;
    	let props = {
    		...this.props,
    		...
    		scrollEventThrottle: needMonitorScroll ? pagescroll : 0,
    	};
    ...
    _childrenWithOverridenStyle () {
    	...
    	return React.Children.map(this.props.children, (child) => {
    		...
    		if (child.type &&
    			...
    			console.warn(`Each ViewPager child must be a <View>. Was 
                ${  child.type.displayName}`);
    		}
    ...
    setPageWithoutAnimation (selectedPage) {
    	...
    	if (this.props.forceScrollView || Platform.OS === "ios"){
    		this.SCROLLVIEW_REF.scrollTo({x: this.state.width * selectedPage, animated: false});
    	} else {
    		this.VIEWPAGER_REF.setPageWithoutAnimation(selectedPage);
    		if (this.props.onPageSelected) {
    ...
    setPage (selectedPage) {
    	this.setState({page: selectedPage});
    	if (this.props.forceScrollView || Platform.OS === "ios") {
    		this.SCROLLVIEW_REF.scrollTo({x: this.state.width * selectedPage});
    	} else {
    		this.VIEWPAGER_REF.setPage(selectedPage);
    		if (this.props.onPageSelected) {
    			this.props.onPageSelected({position: selectedPage})
    			;
    		}
