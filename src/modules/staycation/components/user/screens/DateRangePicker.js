...
export default class DateRangePicker extends Component {
  ...
  populate = () => {
  	const dayHeaders = this.populateHeaders();
  	const weeks = this.populateWeeks();
  	this.setState({
  		dayHeaders,
  		weeks,
  	});
  ...
  populateHeaders = () => {
  	...
  	for (let i = 0; i <= 6; ++i) {
  		const day = moment(displayedDate)
  			.day(i)
  			.format("dddd")
  			.substr(0, 2);
  		dayHeaders.push(
  ...
  populateWeeks = () => {
  	...
  	for (let i = 1; i <= daysInMonth; ++i) {
  		const date = moment(displayedDate).set("date", i);
  		const selected = this.selected(date);
  		const disabled = this.disabled(date);
  		const day = this.generateDay(i, selected, disabled);
  		week.push(day);
  ...
  select = (day) => {
  	...
  	const date = moment(displayedDate);
  	date.set("date", day);
  	if (range) {
  		if (selecting) {
  					...
  					<View style={mergedStyles.container}>
  						<View style={styles.header}>
  							<TouchableOpacity onPress={this.previousMonth}>
  								{monthPrevButton || (
                    <Icon size={mergedStyles.monthButtons.fontSize}
                    color={mergedStyles.monthButtons.color}
                    name="chevron-left" type="evilicon" />
  								)}
  							</TouchableOpacity>
