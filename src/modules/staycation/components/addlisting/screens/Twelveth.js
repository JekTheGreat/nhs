...
class TwelvethScreen extends Component{
	constructor(props){
            super(props);
            this.array = [{
                  newRules: "",
	...
	checkItem = (item, index) => {
		...
		this.setState({isChecked, error: {}});
            console.log("INDEXMOTHERFUCKER: ", isChecked);
	...
	_renderItem = ({item, index}) => {
		...
		const params = setInputDetails.twelveth;
            ...
            const selected = params[item]? params:!params;
            // console.log("INDEX: ",item, index, params, !params[index]);
		return (
            <CheckBox
                  style={{marginTop:20}}
      ...
      _onDelete = (i) => {
            ...
            var index = array.indexOf(i.target.value); 
            delete array[index];
            this.setState({
            arrayHolder: array
            })

            console.log("INDEXMOTHERFUCKER: ", array);
      }
