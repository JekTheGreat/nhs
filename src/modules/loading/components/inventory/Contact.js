...
class Contact extends React.PureComponent{
	...
	onMobilePress = (item) => {
		const {loading: {setInputInventory, selectedInventory, selectedWallet, checkPrefixes},
		...
		if (_.toString(countryData[0].currency) === "PHP" &&
			...
			Alert.alert("Invalid Number", "Please select a valid mobile number in the Philippines");
		}
	}
  ...
  renderItem = ({item, index}) => {
	...
	render(){
		const {visible, onClose, loading: {isSearchingInventory}} = this.props;
  		...
  		<Modal
  			...
  			<SafeAreaView style={styles.container}>
  				...
  				<View style={styles.container}>
					...
					<LoaderModal loading={isSearchingInventory}/>
  			</SafeAreaView>
  		</Modal>
  	);
	}
