...
class InfoScreen extends React.PureComponent{
	...
	renderFlight = (item, index) => {
  	...
  	const source = {uri: `https://storage.googleapis.com/v3-pub-images/ticketing/airlines/${item.OperatingAirline.Code}.png`};
		const DepartTerminal = _.isEmpty(DepartureAirport.TerminalID) ? " " : this.getTerminalWord(DepartureAirport.TerminalID.trim());
		const ArrivalTerminal = _.isEmpty(ArrivalAirport.TerminalID) ? " " : this.getTerminalWord(ArrivalAirport.TerminalID.trim());
  ...
  renderReturn = () => {
  	...
  	return (
  		<View >
  			<Dash style={[styles.dash, styles.marT15]} dashThickness={1} dashLength={10} dashGap={5}	dashColor={Color.Header} />
  			...
  			<View style={styles.marT15}>
  				...
  				<Text style={[styles.txtTotal, styles.marT5, styles.marB15]}>
  					<Text style={styles.fontSize13}>{Currency} </Text>
  					{numeral(TotalAmount + TotalBaggage + TotalSeat).format("0,000.00")}</Text>
  			</View>
  		</View>
	...
	render(){
  	const {open} = this.state;
		const height = open ? null : {overflow: "hidden", height: 40};
  	
		return (
			<TouchableWithoutFeedback onPress={this.onOpen}>
				<View style={[styles.view1, styles.marT15, height]} >
					{!open && <View style={styles.viewAbsolute}/>}
					{this.renderDepart()}
					{this.renderReturn()}
					{this.renderTaxes()}
					<View style={styles.viewArrow}>
						<Icon onPress={this.onOpen} name="chevron-up" type="evilicon" color={Color.LightBlue5} size={40} />
					</View>
				</View>
...
const styles = StyleSheet.create({
	view1: { backgroundColor: "white", marginHorizontal: 25,
	...
	flex1: {flex: 1},
	dash: {width: "100%", height: 1, marginTop: 10},
	viewLayover: {flexDirection: "row", alignItems: "center", justifyContent: "space-between"},
	viewArrow: {alignItems: "center", justifyContent: "center", bottom: -10},
	viewAbsolute: {position: "absolute", ...StyleSheet.absoluteFill, zIndex: 1, elevation: 1, backgroundColor: "white"},
