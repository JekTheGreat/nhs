...
export default class UploadID extends PureComponent{
	...
	componentDidMount(){
		...
		actions.getKYCVerification(isRegistered.token);
	...
	renderReject = () => (
  	<View style={styles.flex1mar30pad30}>
  		<View style={styles.flex1}>
  			...
  			<Text style={styles.txtsuccess}>ID and Selfie with ID Verfied is rejected.</Text>
  		</View>
			<View style={styles.marb20}>
				<Button
	...
	checkAccount = () => {
		const {register: {isRegistered}, actions} = this.props;
	...
	info = (ids) => {
		const info = _.isEmpty(ids) ?
			...
			"Take a photo of yourseld holding up your ID that you are using on upload ID. Make sure the text on your ID on selfie are clearly visible";
  ...
  _handleCurrentStep = () => {
  	...
  	actions.currentStep("proofOfAddressUpload");
  }
  ...
  renderSteps =() => {
	...
	_handleSubmitIdAndSelfie = () => {
		const { actions,  profile: { doneUploadingId, doneUploadingSelfie },
			register: { isRegistered } } = this.props;
		const { kycInputs } = this.state;
		const error1 = {};
		...
		if (_.isEmpty(error1)) {
			actions.verifyUploadIdSelfiePhoto(doneUploadingId, doneUploadingSelfie, kycInputs, isRegistered.token);
		} else {
			this.setState({error: error1});
		}
	}
