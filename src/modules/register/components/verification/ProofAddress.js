	...
	openGalery = (type) => {
		...
		ImagePicker.launchImageLibrary(options, (response) => {
			if (response.didCancel) {
				...
				this.setState({ kycInputs: {...newInput}, error });
			}
		});
	}
	...
	openCamera = (type) => {
		...
		ImagePicker.launchCamera(options, (response) => {
			if (response.didCancel) {
				...
				const filename = response.uri.split("/");
				...
				actions.uploadAddressPhoto(fileupload);
				this.setState({ kycInputs: {...newInput}, error });
			}
		});
	}
