...
import Wallet from "__src/resources/svg/Wallet";
import Profile from "__src/resources/svg/Profile";
...
export default class StaticTabbar extends React.PureComponent {
  ...
  _renderIconActive = ({ route: {routeName}}) => {
  	...
  	switch (routeName){
  	case "Home":
  		return <Home size={28} isActive/>;
  	case "Inbox":
  		return <Inbox size={28} isActive/>;
  	case "Wallet":
  		return <Wallet size={28} isActive/>;
  	case "Notification":
  		return <Notification size={28} isActive/>;
  	case "Account":
  		return <Profile size={28} isActive/>;
  	}
  }
  					...
  					return (
  						<React.Fragment key={`key${key}`}>
  							...
  							<TouchableWithoutFeedback
  								...
  								<Animated.View
  									style={[styles.activeView, {left: tabWidth * key,
