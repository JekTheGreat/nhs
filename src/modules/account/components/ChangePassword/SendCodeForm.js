...
class SendCodeForm extends PureComponent {
	...
	_handleChangeRadio = (type) => {
		...
		if (type === "SMS"){
			actions.changePasswordSaveRadio({
				type,
	...
	_proceedCodeForm = () => {
		...
		if (_.isEmpty(changePasswordSaveRadio)) {
	...
	render() {
		...
		const { sendingCodeChangePass, changePasswordSaveRadio } = account;
		const { additionalDetails } = login;
				...
				<View style={styles.flex1}>
					...
					<View style={styles.marginTop30}>
						<CheckBox
							...
							checked={!!_.isEqual(changePasswordSaveRadio.type, "MAIL")}/>
						<CheckBox
							containerStyle={styles.containerStyle}
							textStyle={styles.textStyle}
							...
							checked={!!_.isEqual(changePasswordSaveRadio.type, "SMS")}/>
